...
import { bookAppointment, cancelAppointment, rescheduleAppointment, checkAvailability, } from "./booking.js";
import { formatSpokenDate, formatSpokenTime, getCurrentDateTime, } from "../helper.js";
...
export async function textYield(msg) {
    ...
    try {
        ...
        if (match) {
            try {
                ...
                if (parsed.tool === "book") {
                    ...
                    const availability = await checkAvailability(parsed.date, parsed.time);
                    ...
                    toolResult = await bookAppointment(parsed.name, parsed.date, parsed.time, `Dental appointment for ${parsed.name}`);
                    if (toolResult.success) {
                        const spokenDate = parsed.spokenDate || formatSpokenDate(parsed.date);
                        const spokenTime = parsed.spokenTime || formatSpokenTime(parsed.time);
                        ...
                        conversation.push({
                            role: "assistant",
                            content: `Tool executed: ${JSON.stringify(toolResult)}. Response: ${confirmMsg}`,
                        });
                    ...
                    const availability = await checkAvailability(parsed.newDate, parsed.newTime);
                    if (!availability.available) {
                        const conflictMsg = `I'm sorry, but that new time slot is already taken. Would you like to try a different time?`;
                        conversation.push({ role: "assistant", content: conflictMsg });
                        return [conflictMsg, false];
                    }
                    toolResult = await rescheduleAppointment(parsed.newDate, parsed.newTime);
                    if (toolResult.success) {
                        const spokenDate = parsed.spokenDate || formatSpokenDate(parsed.newDate);
                        const spokenTime = parsed.spokenTime || formatSpokenTime(parsed.newTime);
                ...
                const errorMsg = "I apologize, but I'm having trouble processing that. Could you please repeat?";
                conversation.push({ role: "assistant", content: errorMsg });
                return [errorMsg, false];
            }
        }
        ...
        // Fallback
        const fallbackMsg = "I'm here to help. Could you tell me what you'd like to do today?";
        conversation.push({ role: "assistant", content: fallbackMsg });
        return [fallbackMsg, false];
    }
    catch (error) {
        console.error("Error in textYield:", error);
        const errorMsg = "I apologize, but I'm experiencing technical difficulties. Please try again.";
        conversation.push({ role: "assistant", content: errorMsg });
        return [errorMsg, false];
    }
}
export const createCall = async (toNum) => {
    try {
...
export const createAudioStreamFromText = async (text, ws, streamSid, isEndCallMsg = false) => {
    ...
    if (isEndCallMsg) {
        await twilioWs.send(JSON.stringify({
            streamSid,
            event: "mark",
            mark: {
                name: "end call",
