...
import { textYield, createCall, createAudioStreamFromText, } from "./controller.js";
...
transcriber.on("turn", async (turn) => {
    ...
    if (turn.transcript && turn.end_of_turn && turn.turn_order != prevTurnOrder) {
        ...
        createAudioStreamFromText(generatedText[0], twilioWs, streamSid, generatedText[1]);
    }
});
...
app.get("/make-call", async (c) => {
    let numberToCall = c.req.query("number") || "";
